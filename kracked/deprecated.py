class KrakenExecutions(BaseKrakenWS):
            ...
            if response["method"] == "add_order":
                ...
                with open(f"session_id_{self.session_id}.csv", "a") as fil:
                    ...
                    fil.write(
                        ",".join(
                            [
                                str(self.order_info[key][self.action_number])
                                for key in self.order_info.keys()
                            ]
                        )
                        + "\n"
                    )
    ...
    def _on_close(self, ws, close_status_code, close_msg):
        print(close_msg)
        print(
            f"WebSocket closed. Status code: {close_status_code}, Message: {close_msg}"
        )
        self.connected.clear()
    ...
    def cancel_order(self, order_id):
        ...
        if order_id == "all":
            ...
            message["params"]["token"] = self.get_ws_token(
    ...
    def add_order(
        ...
        ), f"Order type must be one of the following: {order_types}"
